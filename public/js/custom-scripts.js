$(document).ready(function () {
    ...
    $('.profile-avatar').hover(function () {
        $('.upload_photo').toggle('hide');
    });

    $('.upload_photo').click(function () {
        $("#photo_uploader").trigger('click');
    });


    $("#photo_uploader").change(function (e) {
        var formData = new FormData();
        formData.append('avatar_photo', $(this)[0].files[0]);
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                $('.avatar_photo').attr('src', '/images/avatars/' + res)

            },
            error: function (err) {
                var html = '<div class="alert alert-danger error_photo_upload" role="alert">Error! Please upload png or jpg type!</div>';
                $('.profile-avatar').prepend(html);
                setTimeout('$(".error_photo_upload").remove()', 2000);
            },
            cache: false,
            contentType: false,
            processData: false
    ...
    $('.calend').datepicker()
        .on('changeDate', function (res) {
            ...
            $.get('/timesheet/' + current_date, function (res) {
                table_daily_timesheet.children().remove();
                ajaxWaitDelete();
                if (!res.error) {
                    ...
                    res.forEach(function (elem) {
                        ...
                        var table_daily_html = '<tr data-id="' + elem.id + '">+' +
                            ...
                            '<td><a data-id="' + elem.id + '" id="edit_daily_timesheet" href="javascript:void(0)">Edit</a>' +
    ...
    $('#delete_all').click(function () {
        ...
        if ($(this).hasClass('delete_active')) {
            if (confirm('Do you want to delete it?')) {
                ...
                $.post('/timesheet/delete', {id: ids_to_delete}, function (res) {
                    if (res.success) {
    ...
    $('body').on('click', '.fa-usd', function () {
        ...
        $.post('/timesheet/billable', {id: id, billable: billable}, function () {
        });
    ...
    $("body").on('click', '.week_range', function (e) {
        ...
        ajaxWait();
        $.get('/weekly', {date: range}, function (res) {
        ...
        $.get('/weeks', {weekly_date: range}, function (res) {
    ...
    $("body").on('click', '#submit_timesheet', function (e) {
        e.preventDefault();
        var period = $('.timesheet_period').text();
        var workload = $('.timesheet_workload').text();
        var duration = $('.timesheet_duration').text();
        var balance = $('.timesheet_balance').text();
        ...
        $.post('/timesheet/send', {
            ...
            updateRangeTimesheet(period[0], period[1]);
        });
    });
    ...
    function updateRangeTimesheet(start = null, end = null) {
        ...
        var submit_button_html = '<td class="submit_timesheet_wrap"><button class="btn btn-success" id="submit_timesheet">Submit</button></td>';
        var statistics_href = $('.statistics_href');
        var submit_timesheet_wrap = '.submit_timesheet_wrap';
        $.get('/timesheet/period', {from: start, to: end}, function (res) {
            for (var item in res) {
                $('.timesheet_' + item).text(res[item]);
            }
            if (statistics_href.length > 0 && res['status'] == 'Open') {
                $(submit_timesheet_wrap).remove();
                statistics_href.parent('td').before(submit_button_html);
            }
            ajaxWaitDelete();
        });

        if (start != 'undefined' > 0 && end != 'undefined') {
            updateStatisticHref(start, end);
        }

        $(submit_timesheet_wrap).remove();
    }

    function updateStatisticHref(start, end) {
        var statistics_href = $('.statistics_href');

        if (statistics_href.length > 0) {
